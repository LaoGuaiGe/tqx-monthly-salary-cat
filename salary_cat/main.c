 ...
 * The 22-frame dance (CAT_SPRITE_W x CAT_SPRITE_H sprites, horizontally
 * slimmed 128->72 cols with a box-filter majority-vote downscale from the
 * reference 128x64 frames) plays while the sprite moves in a three-phase
 * loop (screensaver style):
 *   Phase A: left -> right  : start (-CAT_SPRITE_W, 8), dx=1 dy=0, until x > 127
 *   Phase B: top -> bottom  : start ((SCR_W-CAT_SPRITE_W)/2, -CAT_SPRITE_H),
 *                             dx=0 dy=1, until y > 63
 *   Phase C: bottom-right -> top-left: start (SCR_W, SCR_H), dx=-1 dy=-1,
 *            until x < -CAT_SPRITE_W, then back to Phase A.
...
int main(void)
{
    int16_t x = -CAT_SPRITE_W, y = 8;  /* Phase A start position */
    int8_t dx = 1, dy = 0;
    uint8_t frame = 0;
    ...
    while (1) {
        ...
        if (tick % (MOVE_INTERVAL_MS / TICK_MS) == 0) {
            ...
            if (phase == 0 && x > 127) {         /* A: moved fully past right edge */
                phase = 1;
                x = (SCR_W - CAT_SPRITE_W) / 2; y = -48; dx = 0; dy = 1; /* B start */
            } else if (phase == 1 && y > 63) {   /* B: moved fully past bottom edge */
                phase = 2;
                x = 128; y = 64; dx = -1; dy = -1; /* C start */
            } else if (phase == 2 && x < -CAT_SPRITE_W) {  /* C: moved fully past top-left */
                phase = 0;
                x = -CAT_SPRITE_W; y = 8; dx = 1; dy = 0;  /* A start */
            }
        }
